...
public class AddressServiceImpl implements IAddressService
{
    ...
    */
    ...
    public Map<String, List<Map<String, Object>>> getContactsAndHotLines(HttpServletRequest request)
    {
        ...
        // 返回集合(一级)
        Map<String, List<Map<String, Object>>> ret_data = new HashMap<String, List<Map<String, Object>>>();
        ...
        for (int i = 0; i < deptAndPost.size(); i++)
        {
            ...
            else
            {
                ...
                if (fen != null && fen.size() > 0)
                {
                    ...
                    {
                        String fenChildren = addressDao.queryChildrenDeptInfo(Integer.parseInt(fen.get(x).get("manage_deptId").toString()));
                        String FC[] = fenChildren.split(",");
                        // 每一个分管部门及其下属部门
                        for (int e = 0; e < FC.length; e++)
                        {
                            AddList(addressDao.getPerEqual(Integer.parseInt(FC[e])), under);
                        }
                    }
                }
            }
        ...
        // 第一层常用联系人
        ret_data.put("topContactList", LC);
        // 第二层热线
        ret_data.put("hotLineList", LH);
        // 第三层 联系人
        ret_data.put("contactList", valueList);
        // 第四层限制数量
        ret_data.put("limitCount", LN);
    ...
    */
    ...
    public Map<String, List<Map<String, Object>>> getDeptsContact(HttpServletRequest request)
    {
        ...
        ret_data.put("deptList", deptList);
        ret_data.put("contactList", contactList);
        ret_data.put("hotLineList", hotLineList);
        return ret_data;
    }
    ...
    */
    ...
    public Map<String, Object> getContactDetail(HttpServletRequest request)
    {
        ...
        Map<String, Object> result = addressDao.getContactDetail(Integer.parseInt(personnel_id));
